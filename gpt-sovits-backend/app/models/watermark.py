...
class Watermark(db.Model):
    ...
    user = db.relationship(
        "User", foreign_keys=[user_id], backref=db.backref("watermarks", lazy=True)
    )
    model = db.relationship(
        "VoiceModel",
        foreign_keys=[model_id],
        backref=db.backref("watermarks", lazy=True),
    )
        ...
        for _ in range(max_attempts):
            ...
            if not existing:
                watermark = cls(
                    ...
                    user_id=user_id,  # 使用 user_id 作为主要关联
                    username=username,  # 存储用户名但不作为外键
                    model_id=model_id,
