...
def create_official_model(model_data, file_paths, creator_id):
    ...
    try:
        ...
        # 修复：验证必须有两个文件路径
        if not file_paths.get("gpt_model_path"):
            raise ValidationError("GPT model file path is required")
        if not file_paths.get("sovits_model_path"):
            raise ValidationError("SoVITS model file path is required")
        ...
        # 验证两个文件都已复制
        if not stored_paths.get("gpt_model_path"):
            raise ValidationError("Failed to save GPT model file")
        if not stored_paths.get("sovits_model_path"):
            raise ValidationError("Failed to save SoVITS model file")
        ...
        model = VoiceModel(
            ...
            model_type="official",
            gpt_model_path=stored_paths["gpt_model_path"],  # 修复：使用正确字段
            sovits_model_path=stored_paths["sovits_model_path"],  # 修复：使用正确字段
