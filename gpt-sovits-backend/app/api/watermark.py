...
def embed_watermark():
    ...
    try:
        ...
        try:
            ...
            if not watermark_service.dependencies_available:
                return (
                    jsonify(
                        create_response(
                            success=False,
                            message=f"Watermark service unavailable. Missing dependencies: {', '.join(watermark_service.missing_deps)}",
                        )
                    ),
                    503,
                )
        except Exception as e:
            current_app.logger.error(f"Watermark service initialization failed: {e}")
            return (
                jsonify(
                    create_response(
                        success=False,
                        message="Watermark service is currently unavailable",
                    )
                ),
                503,
            )
        ...
        try:
            ...
            raise ValidationError(
                f"Audio file validation failed: {str(e)}", "audio_file"
            )
...
def verify_watermark():
    ...
    try:
        ...
        try:
            ...
            current_app.logger.error(f"Watermark service not available: {e}")
            raise ServiceUnavailableError("Watermark service is not available")
    ...
    try:
        try:
            ...
            active_users = (
                ...
                .filter(
                    Watermark.is_active == True, Watermark.last_used >= thirty_days_ago
                )
                .distinct()
                .count()
            )
