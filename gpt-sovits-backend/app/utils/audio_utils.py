...
def validate_audio_content(file_path):
    ...
    try:
        ...
        if LIBROSA_AVAILABLE:
            try:
                ...
                validation_errors.append(f"Librosa validation failed: {librosa_error}")

    except ImportError:
        validation_errors.append("Librosa not available")
    ...
    if PYDUB_AVAILABLE:
        try:
            ...
            validation_errors.append(f"Pydub validation failed: {pydub_error}")
    else:
        validation_errors.append("Pydub not available")
    ...
    try:
        ...
        # 修复：记录警告但不返回默认值
        warning_msg = f"Could not validate audio content properly. Errors: {'; '.join(validation_errors)}"
        try:
        ...
        return {
            ...
            "channels": 1,
            "is_valid": True,
            "validation_method": "basic_file_check",
            "validation_warnings": validation_errors,
        }

    except Exception as basic_error:
        # 修复：如果连基础检查都失败，抛出明确错误
        all_errors = validation_errors + [f"Basic file check failed: {basic_error}"]
        raise AudioProcessingError(
            f"Audio file validation failed completely. Errors: {'; '.join(all_errors)}"
        )
...
def get_audio_info(file_path):
    ...
    if PYDUB_AVAILABLE:
        try:
            ...
            errors.append(f"Pydub failed: {e}")

    # 使用librosa获取详细信息（如果pydub失败）
    if LIBROSA_AVAILABLE and not info.get("duration"):
        try:
    ...
    # 如果没有获取到基本信息，根据文件大小估算
    if not info.get("duration"):
        if errors:
            warning_msg = f"Could not read audio info. Errors: {'; '.join(errors)}"
            try:
                current_app.logger.warning(warning_msg)
            except RuntimeError:
                print(f"Warning: {warning_msg}")
        ...
        estimated_duration = max(1.0, file_size / (16000 * 2))  # 假设16kHz, 16bit
