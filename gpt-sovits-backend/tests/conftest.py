...
@pytest.fixture
def sample_model_factory(app, admin_user, sample_tags):
    """创建示例模型工厂 - 用于需要动态创建模型的测试"""

    def _create_model(name_suffix=""):
        with app.app_context():
            model = VoiceModel(
                name=f"Test Voice Model{name_suffix}",
                description="A test voice model for testing purposes",
                model_type="official",
                model_path=f"/test/path/model{name_suffix}.pth",
                config_path=f"/test/path/config{name_suffix}.json",
                status="active",
                is_public=True,
                is_featured=True,
                quality_score=8.5,
